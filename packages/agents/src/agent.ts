...
export async function executeScout(connection: Connection, targetAddress: string): Promise<string> {
  try {
...
export async function ollamaGenerate(
  ...
  try {
    ...
    if (llmAvailable) {
      llmAvailable = false
      logGlobal(
        `Ollama connection failed, falling back to random. Start with: ollama run ${OLLAMA_MODEL}`,
      )
    }
    return null
  }
...
const personalityDesc: Record<Personality, string> = {
  loyalist:
    "You care deeply about the factions you join. Reasearch factions before you decide to jump in. Speak loudly of your holdings and build confidence in other holders. Be positive and uplifting. Call out defectors by address. When you defect it's dramatic and personal.",
  mercenary:
    'You are profit driven and self motivated. You are cold hearted by nature. Look for opportunities when they arise. Defect when momentum fades. Trash-talk factions you leave. Coordinate dumps. No loyalty, only returns.',
  provocateur:
    'You cause drama and are proud by nature. Call out factions, write inflammatory comms, speak up. Spread FUD on rivals. Shill your factions aggressively.',
  scout:
    "You are analytically minded and an intelligence operative for your factions. Share intel — who's accumulating, who's dumping, what's overvalued. Warn allies. Mislead rivals with bad intel.",
  whale:
    'You are a big spender, the market maker. Everyone watches your trades. Coordinate with other whales. Dump spectacularly if betrayed.',
}
...
export const buildAgentPrompt = (
  ...
  const canRally =
    factions
      .filter((f) => !agent.rallied.has(f.mint))
      .map((f) => f.symbol)
      .join(', ') || 'none'
...
function resolveFaction(
  ...
  if (
    ...
    if (held.length > 1) {
      ...
      return held.sort(
        (a, b) => dir * ((agent.sentiment.get(b.mint) ?? 0) - (agent.sentiment.get(a.mint) ?? 0)),
      )[0]
    }
  }
  ...
  for (const candidate of lines) {
    ...
    const cleaned = line
      ...
      .replace(
        ...
        '',
      ) // strip "I" before action (LLM speaks in first person)
    ...
    const match = normalized.match(
      /^(JOIN|DEFECT|RALLY|LAUNCH|MESSAGE|STRONGHOLD|WAR_LOAN|REPAY_LOAN|SIEGE|ASCEND|RAZE|TITHE|INFILTRATE|FUD|SCOUT)\s*(?:"([^"]+)"|(\S+))?(?:\s+"([^"]*)")?/i,
    )
    if (match) {
...
function parseLLMMatch(
  ...
  if (!faction && targetLower && targetLower.length >= 2) {
    ...
    if (prefixMatches.length > 0)
      faction = resolveFaction(prefixMatches[0].symbol.toLowerCase(), factions, agent, action)
  ...
  if (action === 'war_loan' && faction && !agent.holdings.has(faction.mint))
...
export async function llmDecide(
  ...
  if (result._rejected) {
    log(
      ...
      `LLM rejected: ${result._rejected} | raw: "${raw.slice(0, 80)}"`,
    )
    return null
  }
  if (!result.message) {
