...
const ACTION_LABELS: Record<string, string> = {
  join: 'join', defect: 'defect', rally: 'rally', launch: 'launch', message: 'message',
  stronghold: 'vault', war_loan: 'war loan', repay_loan: 'repay', siege: 'siege',
  ascend: 'ascend', raze: 'raze', tithe: 'tithe', infiltrate: 'infiltrate', fud: 'fud',
}
...
function categorizeFromLogs(logs: string[]): string {
  ...
  if (logs.some(l => l.includes('Instruction: HarvestFees') || l.includes('SwapFeesToSol'))) return 'tithe'
  return 'unknown'
}
...
function classifyPersonality(weights: number[]): Personality {
  ...
  const scores: Record<Personality, number> = {
    loyalist: msgRatio * 3 + joinRate * 2 + rallyRate * 3 + titheRate * 2 - fudRatio * 3 - defectRate * 2,
    mercenary: defectRate * 3 + infiltrateRate * 3 + warLoanRate * 2 + siegeRate * 2 - msgRatio * 2 - rallyRate * 2,
    provocateur: fudRatio * 4 + infiltrateRate * 2 + defectRate * 1.5 - msgRatio * 1 - rallyRate * 1,
...
export default function AgentPage() {
    ...
    try {
      ...
      while (fetched < MAX_SIGS) {
        const sigs = await connection.getSignaturesForAddress(
          pubkey, { limit: Math.min(100, MAX_SIGS - fetched), before }, 'confirmed',
        )
        ...
        for (let i = 0; i < sigStrings.length; i += 50) {
          ...
          try {
            txs = await connection.getParsedTransactions(batch, { maxSupportedTransactionVersion: 0 })
          } catch { continue }
          ...
          for (const tx of txs) {
            ...
            if (!accountKeys.includes(TORCH_PROGRAM_ID) && !logs.some(l => l.includes(TORCH_PROGRAM_ID))) continue
  ...
  const fetchProfile = useCallback(async () => {
    setLoading(true)
    try {
      const p = await getAgentProfile(connection, address)
      setProfile(p)
    } catch {
      setProfile(null)
    } finally {
  ...
  const fetchComms = useCallback(async () => {
    ...
    try {
      ...
      await batchProcess(pyreFactions.slice(0, 30), async (faction) => {
        const msgs = await getComms(connection, faction.mint, 100, faction.status)
            ...
            <>
              ...
              <div style={{ padding: '0.25rem', margin: '0.25rem' }}>
                <h2 className="text-sm font-medium mb-2" style={{ color: 'var(--muted)' }}>
                  weight distribution {personalityLoading ? '' : personalityData ? `(${personalityData.actionCount} actions)` : ''}
                </h2>
                {personalityLoading ? (
                  ...
                  <p className="text-xs" style={{ color: 'var(--muted)' }}>No on-chain actions found</p>
                ) : (
                    ...
                    {personalityData.weights.map((count, i) => {
                      ...
                      return (
                        <div key={ALL_ACTIONS[i]} className="flex items-center gap-2 text-xs">
                          <span className="w-16 text-right font-mono" style={{ color: 'var(--muted)' }}>
                            {ACTION_LABELS[ALL_ACTIONS[i]]}
                          </span>
                          <div className="flex-1 h-3 rounded-sm overflow-hidden" style={{ background: 'var(--surface)' }}>
                            <div
                          ...
                          <span className="w-6 text-right font-mono" style={{ color: 'var(--muted)' }}>
                            {count}
                          </span>
                        </div>
                      )
              ...
              <div style={{ padding: '0.25rem', margin: '0.25rem' }}>
                ...
                {factionsExpanded && (
                  !profile || profile.factions_joined.length === 0 ? (
                    <p className="text-xs" style={{ color: 'var(--muted)' }}>No faction positions</p>
                  ) : (
                    <div>
                        ...
                        .map((f) => (
                          ...
                          >
                            <div className="flex items-baseline gap-2">
                              ...
                              <span className="font-mono" style={{ color: 'var(--muted)' }}>{f.symbol}</span>
                            </div>
                            <div className="flex items-baseline gap-3">
                              <span style={{ color: 'var(--muted)' }}>{f.percentage.toFixed(2)}%</span>
                      ...
                      {profile.total_value_sol > 0 && (
                        <div className="flex justify-end text-xs mt-2" style={{ color: 'var(--muted)' }}>
