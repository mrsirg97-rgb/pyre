import { getFactionLeaderboard, getRegistryProfile, getAgentFactions } from 'pyre-world-kit'
...
export const buildAgentPrompt = (
  ...
  const memoryBlock =
    chainMemories && chainMemories.length > 0
...
function parseLLMDecision(
  ...
  for (const candidate of lines) {
    ...
    const match = normalized.match(
      /^(JOIN|DEFECT|RALLY|LAUNCH|MESSAGE|STRONGHOLD|WAR_LOAN|REPAY_LOAN|SIEGE|ASCEND|RAZE|TITHE|INFILTRATE|FUD)\s*(?:"([^"]+)"|(\S+))?(?:\s+"([^"]*)")?/i,
    )
    if (match) {
...
export async function llmDecide(
  ...
  // Refresh holdings from on-chain (wallet + vault)
  try {
  ...
  const holdingSummary = [...agent.holdings.entries()]
    .map(([m, b]) => {
      const f = factions.find((ff) => ff.mint === m)
